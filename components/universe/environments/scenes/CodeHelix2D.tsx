...
interface CodeHelix2DProps {
  tier: DeviceTier;
  reduced: boolean;
}

export default function CodeHelix2D({
  tier,
  reduced,
}: CodeHelix2DProps) {
  ...
  const knowledge = getRealmKnowledge("code-helix");
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // Draw helix connection line pathways
      ...
      // Signals between landmarks based on steps
      if (!reduced) {
        ...
        const forge = getCoord("code-forge");
        const compiler = getCoord("compiler-engine");
        const factory = getCoord("ds-factory");
        const arena = getCoord("algo-arena");
        const runtime = getCoord("runtime-engine");

        if (activeSimStep === 0) {
          // code compile call
          const progress = (t * 0.8) % 1.0;
          const tx = forge.x + (compiler.x - forge.x) * progress;
          const ty = forge.y + (compiler.y - forge.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        } else if (activeSimStep === 1) {
          // AST structure layout compile output
          const progress = (t * 0.8) % 1.0;
          const tx = compiler.x + (factory.x - compiler.x) * progress;
          const ty = compiler.y + (factory.y - compiler.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        } else if (activeSimStep === 2) {
          // data structures array routing to algo arena
          const progress = (t * 0.8) % 1.0;
          const tx = factory.x + (arena.x - factory.x) * progress;
          const ty = factory.y + (arena.y - factory.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        } else if (activeSimStep === 3) {
          // Execution stack frame allocation
          const progress = (t * 0.8) % 1.0;
          const tx = arena.x + (runtime.x - arena.x) * progress;
          const ty = arena.y + (runtime.y - arena.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      landmarks.forEach((lm) => {
        ...
        ctx.fillStyle = isActive ? "#ffffff" : isHovered ? "#EEF2F6" : "#7C3AED";
        ctx.beginPath();
