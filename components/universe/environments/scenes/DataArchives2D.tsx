...
interface DataArchives2DProps {
  tier: DeviceTier;
  reduced: boolean;
}

export default function DataArchives2D({
  tier,
  reduced,
}: DataArchives2DProps) {
  ...
  const knowledge = getRealmKnowledge("data-archives");
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // Grid
      ctx.strokeStyle = "rgba(125, 211, 252, 0.03)";
      ctx.lineWidth = 1;
      const gridSize = 45;
      ...
      // Signals between nodes based on simulation step
      if (!reduced) {
        ...
        const query = getCoord("query-engine");
        const index = getCoord("index-towers");
        const vault = getCoord("crystal-vault");

        if (activeSimStep === 0) {
          // query parsing
          const progress = (t * 0.8) % 1.0;
          const tx = query.x + (index.x - query.x) * progress;
          const ty = query.y + (index.y - query.y) * progress;
          ctx.beginPath();
          ...
          // index lookup
          const progress = (t * 0.8) % 1.0;
          const tx = index.x + (vault.x - index.x) * progress;
          const ty = index.y + (vault.y - index.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      landmarks.forEach((lm) => {
        ...
        ctx.fillStyle = isActive ? "#ffffff" : isHovered ? "#F8FAFC" : "#7DD3FC";
        ctx.beginPath();
