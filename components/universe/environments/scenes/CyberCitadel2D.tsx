...
interface CyberCitadel2DProps {
  tier: DeviceTier;
  reduced: boolean;
}

export default function CyberCitadel2D({
  tier,
  reduced,
}: CyberCitadel2DProps) {
  ...
  const knowledge = getRealmKnowledge("the-citadel");
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // Draw security defense pathways
      ...
      // Signals between security nodes
      if (!reduced) {
        ...
        const scanner = getCoord("threat-scanner");
        const auth = getCoord("auth-gate");
        const encrypt = getCoord("encryption-chamber");

        if (activeSimStep === 0) {
          // threat scanning signal
          const progress = (t * 0.8) % 1.0;
          const tx = scanner.x + (auth.x - scanner.x) * progress;
          const ty = scanner.y + (auth.y - scanner.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        } else if (activeSimStep === 1) {
          // auth signature clearance
          const progress = (t * 0.8) % 1.0;
          const tx = auth.x + (encrypt.x - auth.x) * progress;
          const ty = auth.y + (encrypt.y - auth.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      landmarks.forEach((lm) => {
        ...
        ctx.fillStyle = isActive ? "#ffffff" : isHovered ? "#F3F4F6" : "#EF4444";
        ctx.beginPath();
