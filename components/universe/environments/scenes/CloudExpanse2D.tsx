...
interface CloudExpanse2DProps {
  tier: DeviceTier;
  reduced: boolean;
}

export default function CloudExpanse2D({
  tier,
  reduced,
}: CloudExpanse2DProps) {
  ...
  const knowledge = getRealmKnowledge("cloud-expanse");
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // Draw cloud relational pathways
      ...
      // Signals between nodes based on active steps
      if (!reduced) {
        ...
        const balancer = getCoord("load-balancer");
        const cluster = getCoord("server-cluster");
        const monitor = getCoord("monitoring-center");

        if (activeSimStep === 0) {
          // incoming request to server
          const progress = (t * 0.8) % 1.0;
          const tx = balancer.x + (cluster.x - balancer.x) * progress;
          const ty = balancer.y + (cluster.y - balancer.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        } else if (activeSimStep === 1) {
          // auto scaling telemetry report
          const progress = (t * 0.8) % 1.0;
          const tx = cluster.x + (monitor.x - cluster.x) * progress;
          const ty = cluster.y + (monitor.y - cluster.y) * progress;
          ctx.beginPath();
          ctx.arc(tx, ty, 4, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      landmarks.forEach((lm) => {
        ...
        ctx.fillStyle = isActive ? "#ffffff" : isHovered ? "#EEF2F6" : "#38BDF8";
        ctx.beginPath();
