 ...
 */
import { SUPABASE_PUBLISHABLE_KEY, authUrl, authHeaders, restUrl, supabaseFetch } from '@/lib/supabase';
...
export async function signIn(email, password) {
  const response = await supabaseFetch(`${authUrl('token')}?grant_type=password`, {
    method: 'POST',
...
export async function refreshSession(session) {
  ...
  try {
    const response = await supabaseFetch(`${authUrl('token')}?grant_type=refresh_token`, {
      method: 'POST',
      headers: { apikey: SUPABASE_PUBLISHABLE_KEY, 'Content-Type': 'application/json' },
...
export async function signOut(session) {
  if (session?.access_token) {
    try {
      ...
      await supabaseFetch(authUrl('logout'), {
        method: 'POST',
...
export async function checkAdminMembership(accessToken) {
  try {
    const response = await supabaseFetch(`${restUrl('portfolio_admins')}?select=*&limit=1`, {
